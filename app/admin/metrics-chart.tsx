  ...
  return (
    <section className="glass-card p-5">
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <select value={metric} onChange={(event) => setMetric(event.target.value as MetricKey)} className="input-premium w-auto">
          {(Object.keys(metricLabels) as MetricKey[]).map((key) => <option key={key} value={key}>{metricLabels[key]}</option>)}
        </select>
        <select value={days} onChange={(event) => setDays(Number(event.target.value) as 7 | 14 | 30)} className="input-premium w-auto">
      ...
      {filtered.length === 0 ? (
        <p className="text-sm text-slate-400">Пока недостаточно данных для графика.</p>
      ) : (
          ...
          {filtered.map((point) => (
            <div key={point.date}>
              ...
              <div className="h-2 rounded-full bg-white/10">
                <div className="h-2 rounded-full bg-gradient-to-r from-amber-300 to-orange-500" style={{ width: `${(point[metric] / max) * 100}%` }} />
              </div>
            </div>
          ))}
        </div>
